...
const StreamSelectionView = () => {
  ...
  const { imdb_id, media_type } = useLocalSearchParams<{
    imdb_id?: string;
  ...
  return (
    <ThemedView style={styles.container}>
      <FlatList
        data={streams}
        style={styles.list}
        ItemSeparatorComponent={() => <ThemedView style={[styles.separator, {backgroundColor: accentColor}]}/>}
        renderItem={(stream) => {
          const description = stream.item.title ?? stream.item.description;
          const [title, ...tokens] = description?.split("\n") ?? [];
          return (
            <ThemedView style={[styles.item, {backgroundColor: backgroundSoft}]}>
              <ThemedText type={TextType.Bold}>{title.replaceAll('.', ' ')}</ThemedText>
              {tokens.map((t, i) => (
